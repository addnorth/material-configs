...
/* eslint-disable @typescript-eslint/no-explicit-any -- CLI filters dynamic generator output */
...
import { getGenerator, validateSlicer } from "../build/generators/registry.js";
...
Options:
  --slicer <id>            Filter by slicer id (see config/slicers.json)
...
Examples:
  ...
  npm run build -- --slicer bambuslicer      # Build one slicer (id from config/slicers.json)
...
async function build(options: BuildOptions) {
  ...
  if (options.release) {
    ...
    const materials = options.material ? [options.material] : await getAllMaterials();
    ...
    for (const material of materials) {
      ...
      for (const slicer of slicers) {
        ...
        try {
          ...
          const configs = await generator.generateAllConfigs(material, slicer, version, {
            verbose: options.verbose,
            dryRun: options.dryRun,
          ...
          for (const config of filteredConfigs) {
            const outputDir = options.output ? `${options.output}/${slicer}` : `output/${slicer}`;
