...
To **drop old GitHub releases and tags** (keep only the newest `v*` tag) and optionally **squash `main` to a single commit**, use **`maintainers/purge-github-artifacts.sh`**. Run **`./maintainers/purge-github-artifacts.sh --dry-run`** first; **`--squash-history`** requires a **force-pushed** branch and coordinated updates for any forks or clones. Needs authenticated **`gh`**.
