...
/* eslint-disable @typescript-eslint/no-explicit-any -- recursive merge */
  ...
  for (const key in source) {
    ...
    if (Array.isArray(source[key])) {
      if (arrayStrategy === "replace") {
        ...
        (result as any)[key] = [...((result[key] as any[]) || []), ...(source[key] as any[])];
      } else if (arrayStrategy === "unique") {
        (result as any)[key] = [
          ...new Set([...((result[key] as any[]) || []), ...(source[key] as any[])]),
        ];
      }
    } else if (
 ...
 */
export function mergeAll<T extends Record<string, any>>(...objects: (T | null | undefined)[]): T {
  let result = {} as T;
