...
/* eslint-disable @typescript-eslint/no-explicit-any -- JSON / exec error paths */
 ...
 */
export function generateChangelogMarkdown(_version: string, commits: string[]): string {
  const lines: string[] = [];
...
export async function generateChangelog(options: ChangelogOptions = {}): Promise<string> {
  ...
  if (!from && !to) {
    ...
    if (version) {
      ...
      if (tags.includes(currentTag)) {
        ...
        // Tag not in repo yet (e.g. release workflow tags after this step). Avoid
        // git log $FROM..$VERSION while VERSION is not a ref yet; use HEAD instead.
        if (tags.length > 0) {
          from = tags[0];
        }
        to = null;
  ...
  return generateChangelogMarkdown(version || "Unreleased", commits);
}
