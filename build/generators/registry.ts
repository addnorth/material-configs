...
/* eslint-disable @typescript-eslint/no-explicit-any -- generator adapters */
...
export interface GeneratorInfo {
  ...
  saveConfig: (_config: any, _outputDir: string, _options?: any) => Promise<string>;
}
...
export function detectFormat(baseConfig: any, filePath?: string): GeneratorType | null {
  ...
  if (
    ...
    Object.keys(baseConfig).some((key) => key.includes("vendor") || key.includes("filament:"))
  ) {
    return "ini";
  }
  ...
  if (
    !Array.isArray(baseConfig) &&
    (baseConfig.compatible_printers || baseConfig.filament_settings_id || baseConfig.filament_type)
  ) {
    return "json";
  }
