...
/* eslint-disable @typescript-eslint/no-explicit-any -- merged Prusa INI blobs */
...
function applyNozzleOverrides(
  ...
  for (const [sectionKey, sectionValue] of Object.entries(config)) {
    if (sectionKey.includes(`${nozzlePattern}nozzle`) || sectionKey.includes(nozzle)) {
      // Merge overrides into this section
...
function applyCombinationOverrides(
  ...
  for (const [sectionKey, sectionValue] of Object.entries(config)) {
    if (
      ...
      (sectionKey.includes(`${nozzlePattern}nozzle`) || sectionKey.includes(nozzle))
    ) {
...
export async function generateConfig(
  ...
  let config = JSON.parse(JSON.stringify(baseConfig)) as Record<string, Record<string, any>>;

  // Load overrides if printer and nozzle are specified
  if (printer && nozzle) {
    const overrides = await loadAllOverrides(material, slicer, printer, nozzle);
